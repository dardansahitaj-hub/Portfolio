function loadAjaxTxt() {
	...
	charge.open("GET", chemin+"/data.txt", true); //true pour async
	charge.send();
}
...
function loadAjaxPhp() {
	...
	charge.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			document.getElementById("titre").innerHTML = this.responseText;
			// var texte = JSON.parse(this.responseText);
			// document.getElementById("titre").innerHTML = texte[0]+" "+texte[1]+" "+texte[2]+" "+texte[3];
		}
	};
	charge.open("POST", chemin+"/data.php", true);
	charge.send();
}

function loadAjaxJQ() {
    // $('#titre').load(chemin+"/data.txt");
    $.post(
        chemin+"/data.php",
        {
            prenom: "Will",
